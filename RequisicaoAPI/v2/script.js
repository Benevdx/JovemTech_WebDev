...
document.getElementById("irParaCadastro").addEventListener("click", function() {
  // TODO: chame mostrarTela() passando a tela de cadastro
...
document.getElementById("btnBuscarCep").addEventListener("click", function() {
  ...
  // Dica: cep.length !== 8
  if (cep.length !== 8) {
    mostrarMensagem(msgCadastro, "CEP deve conter 8 caracteres.", "erro");
    return;
  }
  ...
  fetch("https://viacep.com.br/ws/" + cep + "/json/")
    ...
    .then(function(dados) {
      ...
      if (dados.erro) {
        mostrarMensagem(msgCadastro, "CEP não encontrado. Verifique o número.", "erro");
        return;
      }
...
document.getElementById("btnEntrar").addEventListener("click", function() {
  ...
  // Se algum estiver vazio, mostre mensagem de erro e use "return"
  if (email === "" || senha === "") {
    mostrarMensagem(msgLogin, "Preencha e-mail e senha para entrar.", "erro");
    return;
  }
  ...
  // Guarde o resultado numa variável chamada "dadosSalvos"
    var dadosSalvos = localStorage.getItem("usuario_" + email);


  // TODO: se dadosSalvos for null (usuário não encontrado),
  // mostre: "E-mail não cadastrado." e use "return"
  if (dadosSalvos === null) {
    mostrarMensagem(msgLogin, "E-mail não cadastrado.", "erro");
    return;
  }
  ...
  if (guardarSenha(senha) !== usuario.senha) {
    mostrarMensagem(msgLogin, "Senha incorreta.", "erro");
    return;
  } 
  if (senha === usuario.senha) {
    mostrarMensagem(msgLogin, "Login bem-sucedido!", "sucesso");
  ...
  avatarInicial.textContent = usuario.nome[0].toUpperCase();
